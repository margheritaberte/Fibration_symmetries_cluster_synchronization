## Taken from https://github.com/HGX-Team/hypergraphx/blob/master/hypergraphx/viz/draw_hypergraph.py
## Added colour hyperedges and nodes according to custom colors 
## Added custom node labels with dictionary
## Added custom edgewidth
## E.g. for cluster synchronization
...
def draw_hypergraph_col(
    ...
    for nid, n in enumerate(list(G.nodes())):
        nx.draw_networkx_nodes(
            ...
            node_color=node_color[nid],
            edgecolors=node_facecolor,
            ax=ax,
        )
        if with_node_labels and label is not None:
            ax.annotate(
                label[n],
    ...
    for nhye, hye in enumerate(list(hypergraph.get_edges())):
        if len(hye) > 2:
            points = []
            for node in hye:
            ...
            plt.fill(
                x1, y1, alpha=hyperedge_alpha, c=edge_color[nhye], edgecolor=edge_color[nhye], zorder = 0
            )
            ...
            facecolor = hyperedge_facecolor_by_order[order]
            plt.fill(
                x1, y1, alpha=hyperedge_alpha, c=edge_color[nhye], edgecolor=edge_color[nhye], linewidth = edge_line_width, zorder = 10
            )
